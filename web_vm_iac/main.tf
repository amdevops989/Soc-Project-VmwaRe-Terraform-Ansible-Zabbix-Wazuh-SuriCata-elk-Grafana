...
# --- VM Resource ---
resource "vmworkstation_vm" "web_vm" {
  sourceid     = "91L6RKBEQS49AKKKEL2ABUFOVURBUA28"
  denomination = "web-vm-${random_id.vm_suffix.hex}"
  description  = "VM managed by Terraform + Triple Playbook Pipeline"
  path         = "/home/ironops/vmware/web-vm-${random_id.vm_suffix.hex}/web-vm-${random_id.vm_suffix.hex}.vmx"
  processors   = 2
  memory       = 2048
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      /home/ironops/Desktop/Soc-Vmware/ansible/playbooks/setup_network.yml
    EOT
  }
  ...
  # STEP 4: Run Nginx Server Playbook (via New Static IP)
  provisioner "local-exec" {
    command = <<EOT
      ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i ${var.vm_static_ip}, \
      --extra-vars "ansible_user=${var.ssh_username} ansible_password=${var.ssh_password} ansible_become_password=${var.ssh_password}" \
      /home/ironops/Desktop/Soc-Vmware/ansible/playbooks/setup_nginx.yml
    EOT
  }
...
# --- Outputs ---
output "vm_id" { 
  value = vmworkstation_vm.web_vm.id 
}

output "vm_name" { 
  value = vmworkstation_vm.web_vm.denomination 
}